...
- **Codex and Gemini join the GUI via the Agent Client Protocol** — the Agent picker in the new-session dialog has three new entries: "ACP: Claude", "ACP: Codex", and "ACP: Gemini". Pick one and you get the same be-conductor chat view — bubbles, tool calls, live streaming, attachments, permission prompts — for Codex or Gemini, not just Claude. Nothing to install up front: the agents are launched on demand (you need Node.js on the machine running be-conductor, and to be signed in to the agent's own CLI). A new [docs/acp.md](docs/acp.md) guide walks through the setup
