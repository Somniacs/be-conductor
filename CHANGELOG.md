...
- **OpenCode model picker in the JetBrains and VSCode plugins** — both new-session dialogs now offer an "Agent" picker for GUI sessions, listing "Claude (native)" alongside every OpenCode model your server is authenticated for ("OpenCode • OpenAI / gpt-5.5", "OpenCode • OpenAI / gpt-5.3-codex", "OpenCode • Google / Gemini …"). Pick a model and the session goes straight through OpenCode — no need to drop to the dashboard for OpenCode sessions
- **Worktree isolation for agent sessions in the dashboard** — the "Isolate with git worktree" checkbox in the new-session dialog used to disappear whenever you picked "Agent" as the session type, even though the backend has always supported it. The checkbox now stays visible for agent sessions (Claude and OpenCode), tool calls run inside the worktree's branch, and you can preview-merge from the worktree view when the session is done. Lets you run multiple agents in parallel on the same repo without conflicts
- **Model name shown inside the message field** — a small read-only label sits at the top-right of the message box like a watermark, always showing which agent + model you're talking to in friendly form: "Claude Opus 4.7", "Claude Sonnet 4.6", "Claude Haiku", "OpenCode • GPT-5.5", "OpenCode • GPT-5.3 Codex", "OpenCode • Gemini 2.5 Pro", etc. Transparent — won't intercept clicks or selection. Updates if the model changes mid-session
- **OpenCode setup guide** — new [docs/opencode.md](docs/opencode.md) walks through installing OpenCode, signing in to a model account (OpenAI, Google, Anthropic), running OpenCode locally or on a remote AI workstation, and creating a session in the dashboard. Linked from the README and the Quick Start guide
...
- **Resumed agent sessions used to lose their chat history on the next restart** — when you resumed a Claude (or any agent) session, the conversation was kept in memory but the on-disk history file got deleted right afterwards. As long as the server stayed up, the chat looked fine. But on the next be-conductor restart, the resumed session showed an empty chat even though the underlying Claude session still had the full conversation. History is now preserved across restarts
- **New-session dialog: cleaner agent picker** — the "OpenCode at <url> — N models" status line below the agent dropdown was always visible, even when "Claude (native)" was selected, where it's irrelevant. It's now hidden unless an OpenCode entry is the current selection. The dropdown labels (which can get truncated by the dialog's narrow width — "OpenCode • OpenAI / gpt-5.3-codex" → "OpenCode • OpenAI / gpt-5...") now show the full text in a tooltip on hover, both for the closed select and for items in the open list
- **Divider drag no longer gets stuck** — dragging the panel divider quickly across an embedded session view (an agent iframe in the dashboard, JetBrains' agent panel, etc.) used to "lose" the cursor to the iframe: the handle would stop following your cursor and the drag wouldn't stop on mouse-up. The handle now captures the pointer for the full drag and temporarily disables iframe pointer-events, so the handle tracks your cursor at any speed
- **Clone hidden for OpenCode sessions** — OpenCode doesn't expose a session-fork operation (its `parent_id` field is just a hierarchy hint, child sessions don't inherit the parent's history), so the Clone button now only appears on Claude agent sessions and on PTY sessions. Same in the dashboard, the JetBrains tool window, and the VSCode session tree. Direct API calls to `/sessions/<id>/clone` for an OpenCode session return a clean 400 with an explanation
...
- **Raw JSON shown as user message in OpenCode sessions** — the very first prompt in an OpenCode session was rendering as a literal `{"type":"prompt","text":"…"}` chat bubble instead of the text the user typed. The orchestrator now accepts the WebSocket route's full call shape so this fallthrough no longer fires
