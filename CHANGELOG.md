...
- **OpenCode as a second GUI agent** — the new-session dialog has an "Agent" picker letting you choose between Claude (the existing native integration) and OpenCode. OpenCode routes to OpenAI/GPT-5.5, codex, gemini, claude-via-API, and any other model you've authenticated via `opencode providers login`. The structured GUI view (chat bubbles, tool calls, streaming, cost) works for either agent — you pick per session in the new-session dialog. Choosing OpenCode also lets you set the model (default `openai/gpt-5.5`)
...
- **Provider-aware UI** — non-Claude sessions show a small badge ("OPENCODE") in the title bar so it's obvious which agent is talking. Claude-only controls (effort dial, adaptive thinking, context ring) are hidden for sessions whose agent doesn't support them, instead of showing as broken or empty
