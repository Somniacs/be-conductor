...
- **The installer can fetch LeanCTX for you** — if `lean-ctx` is missing and Rust is available, the installer offers to build it. The question waits 30 seconds and then answers itself with "no", and it is asked after the server is already back up, so an unattended update is never delayed or left with the server down. be-conductor still installs nothing without a yes
