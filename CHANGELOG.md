...
- **Pick the model from a list** — a profile's Model field is a searchable dropdown of what that account can actually reach (379 entries on an OpenRouter key), not a model id you have to know by heart. Typing or pasting still works, and the field is only a fallback: a run that names its own model always wins. It now shows for every backend, since all of them take a per-run model
