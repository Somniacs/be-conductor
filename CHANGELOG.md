...
- **Pick the model from a list** — a profile's Model field is a searchable dropdown instead of a model id you have to know by heart. For a saved profile it lists what that account can actually reach — including Codex, which has no list command but caches the models its login may use; while you are still adding one it lists the backend's catalogue, narrowed to the provider whose API key variable you named (`OPENROUTER_API_KEY` → 371 OpenRouter models). Typing or pasting still works, and the field is only a fallback: a run that names its own model always wins. It now shows for every backend, since all of them take a per-run model
