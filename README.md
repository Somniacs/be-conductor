...
- **Notes** — scoped scratchpad (global, project, session). Scope combo box with icons, filter chips for quick toggling, push notes into terminal sessions, inline editing, markdown export, multi-client sync via WebSocket. Multi-server: notes from all machines are merged with server labels
- **File viewer** — browse project files, view text with line numbers, render Markdown, display images, preview PDFs, download files. Native diagram rendering for `.drawio`, `.mermaid`/`.mmd`, and `.svg` with interactive zoom/pan (scroll-wheel at cursor, pinch-to-zoom on touch, click-drag panning, fit-to-view, background toggle). Collapsible sidebar with handle, mobile-optimized layout with full-screen file list. Clickable file paths in console output open the viewer directly
