...
async def clipboard_read():
    """Read the OS system clipboard.

    Used by the agent view in JCEF (JetBrains) because JCEF's internal
    clipboard doesn't sync with the OS clipboard. The Python server has
    access to the real clipboard via standard shell tools.
    """
    ...
    try:
        if system == "Linux":
            ...
            result = subprocess.run(["pbpaste"], capture_output=True, timeout=2, check=False)
            if result.returncode == 0:
                text = result.stdout.decode("utf-8", errors="replace")
        elif system == "Windows":
    ...
    try:
        if system == "Linux":
            ...
            subprocess.run(
                ...
                input=text.encode("utf-16-le"), timeout=2, check=False,
            )
    except Exception:
        pass
